...
public class ConwayGUI {
	...
	private void initialize() {
		...
		this.frame.setLocation(this.screenWidth - this.frame.getHeight() / 2,
				this.screenHeight - this.frame.getWidth() / 2);
	...
	private void paintGrid() {
		if (this.frameHolder != null)
			this.frame.remove(this.frameHolder);
		...
		for (int i = 0; i < game.getGeneration().length; i++) {
			for (int j = 0; j < game.getGeneration()[i].length; j++) {
				...
				panel.setSize(screenHeight / game.getGeneration().length,
						screenHeight / game.getGeneration()[0].length);
				panel.setPreferredSize(new Dimension(screenHeight / game.getGeneration().length,
						screenHeight / game.getGeneration()[0].length));
				c.gridx = i;
				c.gridy = j;
